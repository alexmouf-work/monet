...
| 2026-08-11 | **Open a Minecraft model from your own files** (owner request): pick the model JSON and Monet lists the textures it references, taking them one by one or in bulk; or point at the folder and they are found relative to the model. Anything you do not have takes the magenta/black placeholder — a real texture you can paint on, not a dead reference. Export → Model bundle (.zip) gives the whole thing back with your edits in it. |
| 2026-08-11 | **Fade one colour** (owner request): Recolour gained an Opacity mode beside Replace and Tint — pick a colour (with a tolerance if you want its neighbours too) and multiply its opacity by 0–255. 255 changes nothing, 128 halves it, 0 makes it invisible; the colour itself is never touched, only how much of it shows. |
| 2026-08-11 | **Number fields can be emptied while you type** (owner report): backspacing the last digit used to leave it there. Clear a field and click away and it settles on a sensible default — 1 for the brush size, 255 for an opacity — instead of snapping back. Opacity is now 0–255 everywhere and you can type it as well as drag it. |
| 2026-08-11 | **Selections rotate and flip while they are still selected** (owner request): with a selection live the scroll wheel turns it — 15° a notch, hold Shift for 1° — and `F` mirrors it top-to-bottom, `Shift+F` left-to-right. It works on a plain marquee too (it lifts it for you), right angles come out pixel-exact, and winding the wheel back gets the original art back rather than something progressively mushier. With nothing selected `F` is still the bucket and the wheel is still the zoom. |
| 2026-08-11 | **Recolour with a tolerance keeps shading** (owner request): swapping a colour now moves the similar ones along with it instead of flattening them all onto one — a dark green recoloured to dark purple takes the very dark green to very dark purple, with the same difference between them. *Flatten to one* is still there as a second option when you do want every matched pixel identical; at 0 % tolerance the choice makes no difference. |
| 2026-08-11 | **What you are doing is now visible while you do it** (four owner reports): a stroke or erase appears as you drag rather than when you release; the brush outline highlights exactly the pixels it will paint; a pasted or dragged selection is drawn where it actually is instead of showing an empty marquee; clicking outside a moved selection puts the pixels down instead of losing them; cut then paste leaves one copy, not two; and copying a region no longer takes the background with it, so empty space stays empty when the block lands on artwork. |
| 2026-08-11 | **Save texture edits back to the folder** (owner request), a toggle **defaulting OFF**: with it on, Ctrl+S on a texture from a folder-loaded model overwrites the file it came from, so the folder becomes a live editing session instead of an import/export round trip. Off, nothing in the folder is touched and the zip is the only way out. Only files Monet actually read from that folder are ever overwritten — a texture added by hand or a placeholder has no home there, so it stays in Monet rather than being scattered into the user's directory. Offered only where the browser hands over real file handles, and write permission is asked when the toggle is flipped, not silently at save time. Changeable from the load dialog or the source in the sidebar. |
