...
| # | Decision | Rationale |
| - | -------- | --------- |
| D1 | **Platform: client-side web app + PWA**, hosted on **Vercel** (owner-confirmed; Vercel chosen 2026-08-09) | Matches the mouftools pattern (static app per subdomain); every required function — jar parsing, painting, exports, GitHub branch/push/merge — works client-side. Canvas performance is identical to a wrapped native app. **Amended 2026-08-10**: the editor still needs no server, but "Sign in with GitHub" does — one stateless Edge Function (`api/github/token.ts`) holds the App's client secret, because GitHub's token endpoint requires it and sends no CORS headers. That single function is the only server-side code; everything else remains static output. |
...
| D5 | **GitHub via REST API**, as a **GitHub App** the user signs into, with a fine-grained PAT as the fallback (App added 2026-08-10 on owner request) | `api.github.com` sends CORS headers, so the browser can create branches, commit+push on save, and fast-forward branches directly — no git binary. Signing in beats a hand-made PAT because the App's reach is bounded by its installation (Contents, on repositories the user picks) and its tokens expire; the PAT route stays for self-hosting and for users who would rather not install an App. Details: docs/08 §4. |
